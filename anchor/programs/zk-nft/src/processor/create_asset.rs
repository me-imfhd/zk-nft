...
pub fn create_asset<'info>(
    ...
    asset_data_params: AssetDataParams,
...
fn create_compressed_pdas<'info>(
    ...
    let base_data = BaseData {
        ...
        delegate: None,
        delegate_role: DelegateRole::None,
    };
    ...
    let asset_data = AssetData {
        mutable: asset_data_params.mutable,
        asset_id: asset_id.into(),
        authority: match &ctx.accounts.asset_authority {
            Some(asset_authority) => Some(asset_authority.key()),
            None => None,
        },
        ...
        has_attributes: attributes_params.is_some(),
...
pub struct CreateAsset<'info> {
    ...
    pub group_authority_or_delegate: Option<Signer<'info>>,
    /// CHECK: This can be any valid public key, but it must be the same as the group's authority if group is provided.
    pub asset_authority: Option<UncheckedAccount<'info>>,
